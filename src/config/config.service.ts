...
export class ConfigService implements OnModuleInit {
  ...
  constructor() {
    ...
    if (fs.existsSync(configFile)) {
      try {
        ...
        this.logger.log(`Configuration TOML chargée depuis : ${configFile}`);
      } catch (err) {
  ...
  get port(): number {
    return parseInt(
      this.resolve('PORT', ['server', 'port'], '3000')!,
      10,
    );
  }
  ...
  private resolve(
    ...
    if (tomlVal !== undefined && tomlVal !== null) {
      this.logger.debug(`${envKey} → depuis config.toml [${tomlPath.join('.')}]`);
